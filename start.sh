...
./dockerd -r . -b /dev -b /sys -b /proc -b /tmp \
    ...
    pip install websockify --break-system-packages && \
     wget -O primeOS-mainline_0.6.1-20211206-k4.19.195.iso https://archive.org/download/prime-os-nougat-last-releases/primeOS-mainline_0.6.1-20211206-k4.19.195.iso && mv primeOS-mainline_0.6.1-20211206-k4.19.195.iso /"
...
run_system() {
  ...
  # start qemu vm
  d.stat "starting Android..."
  d.stat "password: admin"
  $DOCKER_RUN "qemu-system-x86_64 -m "4096" -smp $(nproc --all) -nic user,hostfwd=tcp::"25052"-:3389 -drive file=primeOS-mainline_0.6.1-20211206-k4.19.195.iso -display vnc=127.0.0.1:1 -device virtio-tablet-pci -display default,show-cursor=on"                         
