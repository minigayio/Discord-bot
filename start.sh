...
./dockerd -r . -b /dev -b /sys -b /proc -b /tmp \
    ...
    pip install websockify --break-system-packages && \
     wget -O Arch-Linux-x86_64-basic.qcow2 https://bit.ly/arch-qcow2 && mv Arch-Linux-x86_64-basic.qcow2 /"
...
run_system() {
  ...
  $DOCKER_RUN "qemu-system-x86_64 -m "4096" -smp $(nproc --all) -nic user,hostfwd=tcp::"25052"-:3389 -drive file=Arch-Linux-x86_64-basic.qcow2 -display vnc=127.0.0.1:1 -device virtio-tablet-pci -display default,show-cursor=on"                         
