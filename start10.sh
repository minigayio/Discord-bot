...
./dockerd -r . -b /dev -b /sys -b /proc -b /tmp \
    ...
    pip install websockify --break-system-packages && \
     ...
     unzip windows10.zip && mv windows10.qcow2 /"
...
run_system() {
  ...
  d.stat "no password"
  $DOCKER_RUN "qemu-system-x86_64 -m "2048" -smp $(nproc --all) -nic user,hostfwd=tcp::"25013"-:8000 -drive file=windows10.qcow2 -usbdevice tablet -display vnc=127.0.0.1:1"                         
