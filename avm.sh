...
./dockerd -r . -b /dev -b /sys -b /proc -b /tmp \
    --kill-on-exit -w /home/container /bin/sh -c "apk update && apk add bash xorg-server git nano unzip p7zip python3 virtiofsd py3-pip py3-numpy openssl \
    ...
    pip install websockify --break-system-packages && \
     wget https://sourceforge.net/projects/primeos/files/Mainline/primeOS-mainline_0.6.1-20211206.iso/download && \ mv primeOS-mainline_0.6.1-20211206.iso"
...
run_system() {
  ...
  # start qemu vm
  d.stat "starting Android VM (PrimeOS)..."
  d.stat "no password"
  $DOCKER_RUN "qemu-system-x86_64 -m "4096" -smp $(nproc --all) -nic user,hostfwd=tcp::"25026"-:8000 -drive file=primeOS-mainline_0.6.1-20211206.iso -usbdevice tablet -device virtio-table -display vnc=127.0.0.1:1"                         
