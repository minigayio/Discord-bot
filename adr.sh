...
./dockerd -r . -b /dev -b /sys -b /proc -b /tmp \
    ...
    pip install websockify --break-system-packages && \
     wget -O android-x86_64-9.0-r2-k49.iso https://bit.ly/adr-x86 && mv android-x86_64-9.0-r2-k49.iso /"
...
run_system() {
  ...
  $DOCKER_RUN "qemu-system-x86_64 -cdrom android-x86_64-9.0-r2-k49.iso -boot menu=on -m 6G -smp 8 -cpu host -enable-kvm -vga virtio -display gtk,gl=on,show-cursor=on -usb -device usb-tablet -device virtio-keyboard -soundhw hda -net nic,model=virtio -net user,hostfwd=tcp::5555-:5555 -machine q35 -accel kvm -full-screen -rtc base=localtime"                         
